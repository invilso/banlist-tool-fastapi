...
class Parser():
    ...
    def parse(self, server: str):
        with open(server+'.json', "w+", encoding="utf-8") as f:
            if server == 'rp2'
                f.write(json.dumps(re.split('<br>', self.forum.getF("http://gta-trinity.ru/"+server+"mon/bans.php", False))[4:50006], sort_keys=True, indent=4))
            else:
                f.write(json.dumps(re.split('<br>', self.forum.getF("http://gta-trinity.ru/"+server+"mon/bans.php", False))[5:50007], sort_keys=True, indent=4))
